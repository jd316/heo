...
async function main() {
    ...
    const hypotheses = await hypothesisService.generateAndScoreHypotheses({ query: userQuery }, context);
    ...
    if (SKIP_DKG) {
        ...
        try {
            ...
            try {
                ...
                const assetAllowance = dkgClient.asset;
            ...
            const datasetRoot = createResult.datasetRoot;
            if (!datasetRoot) {
            ...
            const finalityResult = await dkgClient.asset.publishFinality(UAL, { minimumNumberOfFinalizationConfirmations: 1 });
    ...
    `;
            ...
            try {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const queryPromise = dkgClient.graph.query(sparqlQuery, 'SELECT');
