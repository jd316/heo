...
class IPFSService {
    ...
    initialize(context) {
        ...
        this.apiUrl = (context.config?.[IPFS_ENDPOINT_CONFIG_KEY] ||
            process.env[IPFS_ENDPOINT_CONFIG_KEY] ||
            DEFAULT_IPFS_API_URL);
        // Determine gateway URL
        this.gatewayUrl = (context.config?.[IPFS_GATEWAY_URL_CONFIG_KEY] ||
        ...
        try {
            ...
            const clientConfig = { url: this.apiUrl };
    ...
    async store(data) {
        ...
        try {
            ...
            throw new ApplicationError('Failed to store content to IPFS', ErrorCode.SERVICE_UNAVAILABLE, { originalError: error });
        }
    }
    async retrieve(hash) {
        if (!this.client) {
        ...
        try {
            ...
            throw new ApplicationError('Failed to retrieve content from IPFS', ErrorCode.SERVICE_UNAVAILABLE, { hash, originalError: error });
        }
    }
    getGatewayUrl(cid, context) {
