...
class SolanaService {
    ...
    keypairPath = DEFAULT_SOLANA_KEYPAIR_PATH;
    ...
    async initialize(context) {
        ...
        try {
            ...
            if (fs.existsSync(this.keypairPath)) {
                ...
                const keypairData = JSON.parse(keypairDataRaw);
                ...
                fs.writeFileSync(this.keypairPath, JSON.stringify(Array.from(this.keypair.secretKey)));
            ...
            if (this.isProduction) {
                throw new ApplicationError('Failed to initialize Solana service', ErrorCode.SERVICE_UNAVAILABLE);
            }
            return false;
        }
    }
    async ensureInitialized(context) {
        if (!this.initialized) {
            ...
            if (!(await this.initialize(context))) {
                throw new ApplicationError('Solana service auto-initialization failed', ErrorCode.SERVICE_UNAVAILABLE);
            }
        }
        if (!this.connection || !this.keypair) { // Should be caught by initialize, but as a safeguard
            ...
            throw new ApplicationError('Solana service critical components missing after initialization', ErrorCode.INTERNAL_ERROR);
        }
    }
    /**
     * Anchor proof data on Solana blockchain
     */
    async anchorProof(protocolInstanceId, proof, ipfsCid, context) {
        ...
        try {
            ...
            throw new ApplicationError('Failed to anchor proof on Solana', ErrorCode.INTERNAL_ERROR, { protocolInstanceId, ipfsCid });
        }
    }
    /**
     * Initialize a protocol on Solana
     */
    async initializeProtocol(templateId, initiatorPublicKey, _parameters, context) {
        await this.ensureInitialized(context);
        const currentLogger = context?.logger || this.logger;
        const connection = this.connection;
        const keypair = this.keypair;
        try {
            ...
            throw new ApplicationError('Failed to initialize protocol on Solana', ErrorCode.INTERNAL_ERROR, { templateId, initiatorPublicKey });
        }
    }
    /**
     * Get SOL balance for public key
     */
    async getBalance(publicKeyStr, context) {
        await this.ensureInitialized(context);
        const currentLogger = context?.logger || this.logger;
        const connection = this.connection;
        try {
            ...
            throw new ApplicationError('Failed to get SOL balance', ErrorCode.INTERNAL_ERROR, { publicKey: publicKeyStr });
        }
    }
    /**
     * Transfer SPL token
     */
    async transferSplToken(recipientPublicKeyStr, tokenMintAddressStr, amountLamports, senderKeypair, context) {
        await this.ensureInitialized(context);
        const currentLogger = context?.logger || this.logger;
        const connection = this.connection;
        ...
        try {
            ...
            const fromTokenAccount = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenMintPublicKey, payer.publicKey);
            // Get or create the token account of the recipient
            const toTokenAccount = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenMintPublicKey, recipientPublicKey);
            ...
            if (!toTokenAccountInfo) {
                ...
                transaction.add(Token.createAssociatedTokenAccountInstruction(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenMintPublicKey, toTokenAccount, recipientPublicKey, payer.publicKey));
            }
            transaction.add(Token.createTransferInstruction(TOKEN_PROGRAM_ID, fromTokenAccount, toTokenAccount, payer.publicKey, [], amountLamports));
            ...
            throw new ApplicationError('Failed to transfer SPL token', ErrorCode.INTERNAL_ERROR, { recipient: recipientPublicKeyStr, tokenMint: tokenMintAddressStr });
        }
    }
    /**
