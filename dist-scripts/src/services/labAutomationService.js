...
export class LabAutomationService {
    ...
    useMock = !process.env.ECL_API_KEY || !process.env.ECL_API_URL;
    apiKey = process.env.ECL_API_KEY;
    baseUrl = process.env.ECL_API_URL;
    initialize(context) {
     ...
     */
    async submitRun(protocolPayload) {
        if (this.useMock) {
        ...
        if (!res.ok)
            throw new Error(`ECL submit failed: ${res.status}`);
     ...
     */
    async pollRunStatus(runId, intervalMs = 5000) {
        if (this.useMock) {
        ...
        while (true) {
            ...
            if (!res.ok)
                throw new Error(`ECL status failed: ${res.status}`);
    ...
    async fetchResults(runId) {
        if (this.useMock) {
            ...
            if (!geminiApiKey)
                throw new Error('Gemini API key is required for mock result generation');
        ...
        if (!res.ok)
            throw new Error(`ECL fetchResults failed: ${res.status}`);
        return res.json();
    }
    /**
