...
const PROTOCOL_CHECK_CIRCUIT_WASM_PATH = process.env.PROTOCOL_CHECK_CIRCUIT_WASM_PATH ||
    'circuits/protocol_check_js/protocol_check.wasm'; // Default path
const PROTOCOL_CHECK_CIRCUIT_ZKEY_PATH = process.env.PROTOCOL_CHECK_CIRCUIT_ZKEY_PATH || 'circuits/protocol_check_final.zkey'; // Default path
const PROTOCOL_CHECK_VERIFICATION_KEY_PATH = process.env.PROTOCOL_CHECK_VERIFICATION_KEY_PATH || 'circuits/verification_key.json'; // Default path
/**
...
class ZkSnarkService {
    ...
    verificationKeyJson; // To cache the loaded verification key, type from snarkjs is unclear
    constructor() {
    ...
    async loadVerificationKey() {
        if (!fs.existsSync(this.verificationKeyPath)) {
            logger.error(`ZkSnarkService: Verification key file not found at ${this.verificationKeyPath}. Proof verification will fail.`);
        ...
        try {
            ...
            logger.info(`ZkSnarkService: Verification key loaded successfully from ${this.verificationKeyPath}`);
        }
        catch (error) {
            logger.error(`ZkSnarkService: Error loading or parsing verification key from ${this.verificationKeyPath}`, { error });
    ...
    async generateProof(params) {
        ...
        try {
            ...
            const ipfsCid = await ipfsService.store(JSON.stringify(params.rawData));
            ...
            if (!fs.existsSync(this.circuitWasmPath) || !fs.existsSync(this.circuitZkeyPath)) {
                ...
                if (_IS_PRODUCTION) {
                    throw new ApplicationError('Circuit files are missing, cannot generate proof.', ErrorCode.SERVICE_UNAVAILABLE);
                }
            ...
            const { proof, publicSignals } = await snarkjs.groth16.fullProve(circuitInputs, this.circuitWasmPath, this.circuitZkeyPath, { info: (msg) => logger.info(msg), debug: (msg) => logger.debug(msg) });
            ...
            throw new ApplicationError('Failed to generate zkSNARK proof', ErrorCode.INTERNAL_ERROR, { protocolInstanceId: params.protocolInstanceId, error: error instanceof Error ? error.message : String(error) });
        }
    }
    /**
    ...
    async verifyProof(proof, publicSignals) {
        ...
        try {
            if (!fs.existsSync(this.verificationKeyPath)) {
                ...
                if (_IS_PRODUCTION) {
                    throw new ApplicationError('Verification key is missing, cannot verify proof.', ErrorCode.SERVICE_UNAVAILABLE);
                }
            ...
            const isValid = await snarkjs.groth16.verify(this.verificationKeyJson, publicSignals, proof);
            logger.info(`ZkSnarkService: Proof verification ${isValid ? 'successful' : 'failed'}`);
            return isValid;
        }
        catch (error) {
            logger.error('Failed to verify zkSNARK proof:', error);
            throw new ApplicationError('Failed to verify zkSNARK proof', ErrorCode.INTERNAL_ERROR, { error: error instanceof Error ? error.message : String(error) });
        }
    }
    /**
     * Generate proof and anchor it on Solana in one operation
     */
    async generateAndAnchorProof(params) {
        const proofResult = await this.generateProof(params);
        const transactionId = await solanaService.anchorProof(params.protocolInstanceId, proofResult.proof, proofResult.ipfsCid);
     ...
     */
    async runGroth16Prover(circuitInputs, // Changed from Record<string, any>
    _context) {
        ...
        if (!fs.existsSync(this.circuitWasmPath)) {
            logger.error(`ZkSnarkService: Circuit WASM file not found at ${this.circuitWasmPath}. Cannot generate proof.`);
            throw new Error(`Circuit WASM file missing. Expected at ${this.circuitWasmPath}`);
        }
        if (!fs.existsSync(this.circuitZkeyPath)) {
            logger.error(`ZkSnarkService: Circuit ZKEY file not found at ${this.circuitZkeyPath}. Cannot generate proof.`);
            throw new Error(`Circuit ZKEY file missing. Expected at ${this.circuitZkeyPath}`);
        }
        try {
            ...
            const { proof, publicSignals } = await snarkjs.groth16.fullProve(inputsForSnarkJs, this.circuitWasmPath, this.circuitZkeyPath);
            const endTime = Date.now();
            logger.info(`ZkSnarkService: Groth16 proof generated successfully in ${endTime - startTime}ms.`, { publicSignals });
            ...
            throw new Error(`Failed to generate Groth16 proof: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    async verifyGroth16Proof(proof, publicSignals, _context) {
        logger.info('ZkSnarkService: Verifying Groth16 proof.', { publicSignals });
        if (!this.verificationKeyJson) {
            logger.error(`ZkSnarkService: Verification key not loaded. Cannot verify proof. Check path: ${this.verificationKeyPath}`);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    _circuitInputs, // Keeping 'any' for mock flexibility, disabled explicit-any for this line
    _context) {
        logger.warn('ZkSnarkService: runMockGroth16Prover is being called. This is a MOCK implementation.');
        ...
        const mockPublicSignals = ['1', '0']; // Example for the ProtocolCheck circuit (valid if hazardous=1, cabinet=1)
     ...
     */
    async verifyMockGroth16Proof(_proof, _publicSignals, _context) {
        logger.warn('ZkSnarkService: verifyMockGroth16Proof is being called. This is a MOCK implementation.');
...
export async function runGroth16Prover(protocolInstanceId, rawData) {
