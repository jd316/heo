...
import { oxigraphCacheService } from './oxigraphCacheService.ts';
...
export const internalHelpers = {
    async loadCorpusData(filters, logger // Use context's logger type
    ) {
        ...
        return [
            {
                ...
                content: "Default mock corpus data entry. This simulates a broad retrieval when no specific filters are applied.",
                metadata: {
    ...
    async fetchContextFromOxigraph(userQuery, contextOrLogger, fallbackLogger) {
        // Determine the logger to use
        const effectiveLogger = (contextOrLogger && 'logger' in contextOrLogger)
    ...
    `;
        try {
            ...
            const uniqueContexts = new Map(); // To store text and its source
    ...
    normalizeQueryToContents: (query) => {
        if (typeof query === "string") {
            return [{ role: "user", parts: [{ text: query }] }];
        }
        if (Array.isArray(query)) {
            return query.map((p) => typeof p === "string" ? { role: "user", parts: [{ text: p }] } : p);
        }
        return [query];
    },
    async callGeminiProService(params, logger) {
        ...
        effectiveLogger.info("callGeminiProService: Calling Gemini Pro service using ai.models.generateContent", {
        ...
        try {
            const genAI = params._genAIInstanceBuilder
                ? params._genAIInstanceBuilder(params.apiKey)
                : new GoogleGenAI({ apiKey: params.apiKey });
            const requestContents = internalHelpers.normalizeQueryToContents(params.prompt);
            ...
            if (typeof textContent !== "string") {
                effectiveLogger.error("callGeminiProService: Invalid API response structure from Gemini or text content missing", { response: result });
                throw new Error("Invalid API response from Gemini service. Expected a text string.");
            }
            ...
            throw new Error(`Error communicating with Gemini service: ${error instanceof Error ? error.message : String(error)}`);
        }
    },
    async callGeminiEmbeddingService(params, logger) {
        const effectiveLogger = logger || console;
    ...
    async scoreNovelty(hypothesesText, apiKey, embeddingModelName, contextQueryForOxigraph, logger) {
        const effectiveLogger = logger || console;
        ...
        if (contextQueryForOxigraph) {
            ...
            const contextTexts = await internalHelpers.fetchContextFromOxigraph(contextQueryForOxigraph, contextObj);
    ...
    async generateRdfTriples(hypothesis, _originalQuery, logger) {
        const effectiveLogger = logger || console;
    ...
    async anchorToIpfs(rdfData, context, logger) {
    ...
    parseHypothesesFromLLMResponse(llmResponseText, maxHypotheses, logger) {
        const effectiveLogger = logger || console;
...
const generateAndScoreHypotheses = async (input, context) => {
    ...
    const noveltyScores = await internalHelpers.scoreNovelty(rawHypothesesTexts, geminiApiKey, embeddingModelName, query, effectiveLogger);
...
class HypothesisService {
    ...
    initialize(context) {
        ...
        this.geminiApiKey = context.config?.GEMINI_API_KEY || process.env.GEMINI_API_KEY;
        this.generationModelName = context.config?.GEMINI_MODEL_NAME_GENERATION || process.env.GEMINI_MODEL_NAME || "gemini-1.5-flash-latest";
        this.embeddingModelName = context.config?.GEMINI_MODEL_NAME_EMBEDDING || "text-embedding-004";
    ...
    async _fetchContextFromOxigraph(userQuery, contextOrLogger, fallbackLogger) {
        // Determine the logger to use
        const effectiveLogger = (contextOrLogger && 'logger' in contextOrLogger)
            ? contextOrLogger.logger
    ...
    _normalizeQueryToContents(query) {
        if (typeof query === "string") {
            return [{ role: "user", parts: [{ text: query }] }];
        }
        if (Array.isArray(query)) {
    ...
    async _callGeminiProService(params) {
        ...
        if (!effectiveModelName)
            throw new Error("Gemini generation model name not configured.");
        try {
            ...
            if (typeof textContent !== "string") {
                this.logger.error("HypothesisService._callGeminiProService: Invalid API response structure from Gemini or text content missing", { response: result });
                throw new Error("Invalid API response from Gemini service. Expected a text string.");
            }
            return [textContent];
        }
    ...
    async _callGeminiEmbeddingService(params) {
        ...
        if (!this.geminiApiKey)
            throw new Error("Gemini API key not configured.");
        if (!effectiveModelName)
            throw new Error("Gemini embedding model name not configured.");
        if (!params.texts || params.texts.length === 0)
            return [];
        const genAI = params._genAIInstanceBuilder && this.geminiApiKey
            ? params._genAIInstanceBuilder(this.geminiApiKey)
    ...
    _cosineSimilarity(vecA, vecB) {
        if (!vecA || !vecB || vecA.length !== vecB.length || vecA.length === 0) {
            this.logger.warn("Invalid vectors for cosine similarity.");
            return 0;
        }
    ...
    async _scoreNovelty(hypothesesText, contextQueryForOxigraph, _contextForNoveltyCheck) {
        if (!this.embeddingModelName)
            throw new Error("Embedding model not configured for novelty scoring.");
        ...
        if (contextQueryForOxigraph) {
            ...
            // Pass the context object to fetchContextFromOxigraph
            const contextTexts = await this._fetchContextFromOxigraph(contextQueryForOxigraph, contextObj);
            if (contextTexts.length > 0) {
    ...
    async _generateRdfTriples(hypothesis, _originalQuery) {
        const { DataFactory } = N3;
        const { namedNode, literal } = DataFactory;
        const writer = new N3.Writer({
            prefixes: {
    ...
    async _anchorToIpfs(rdfData, _context) {
    ...
    _parseHypothesesFromLLMResponse(llmResponseText, maxHypotheses) {
        ...
        for (const line of lines) {
            ...
            if (cleanedLine.length > 20 && cleanedLine.includes(' ') && !cleanedLine.toLowerCase().startsWith("reasoning:") && !cleanedLine.toLowerCase().startsWith("experiment type:")) { // Basic filter
                ...
                if (uniqueHypotheses.size >= maxHypotheses)
                    break;
            }
        }
    ...
    async generateAndScoreHypotheses(input, context) {
